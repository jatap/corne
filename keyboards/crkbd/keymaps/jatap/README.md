...
- Add `--save` to write the output to files instead of the chat: `/layers apps --save` writes `docs/apps.md`, and `/layers --save` writes one file per layer. File names use the lowercase layer name, and existing files are overwritten. Each saved file starts with a `# <Layer name>` heading and contains a key table and a layout table. Every table has an ASCII `+`/`-` border line above its header and below its last row, uses ASCII `|` delimiters, escapes a literal pipe key as `\|`, keeps every one of its lines the same length within 160 columns, and contains no box-drawing characters. The key table uses a `| --- |` separator row directly under its header. The layout graph table instead uses a border line under its header and another above its `THUMBS` row. Never run Prettier on `docs/*.md`: it folds a border line into the first column and re-pads cells, which destroys the borders.
